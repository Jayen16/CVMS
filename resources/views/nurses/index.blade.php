        ...
        <div x-show="inviteOpen" x-cloak x-on:keydown.escape.window="inviteOpen = false" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="invite-staff-title">
                <div class="app-panel w-full max-w-lg" @click.stop>
                    ...
                    <form method="POST" action="{{ route('nurses.store') }}" class="mt-5 grid gap-4">
                        @csrf
                        <p class="text-sm text-slate-600 dark:text-zinc-300">{{ config('system.instance_type') === 'facility' && config('offline.enabled') ? 'A local password setup link will appear after you create this account. Copy it and open it on the user\'s computer.' : ($managedRole === 'barangay_admin' ? 'The Barangay Admin receives an email link to set their password. They can manage nurses for their assigned barangay.' : 'The nurse receives an email link to set their password. Until then, the account stays pending.') }}</p>
                        <x-form-field label="Name" name="name" />
                        <x-form-field label="Email" name="email" type="email" />
                        <x-form-field label="Phone number (optional offline)" name="phone" type="tel" />
