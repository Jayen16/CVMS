@props(['label', 'name', 'options' => [], 'value' => null, 'disabled' => false])
...
<div
    ...
    x-data="{ open: false, disabled: @js($disabled), filtering: false, submitTimer: null, search: @js($selected['label'] ?? ''), selected: @js((string) ($value ?? '')), items: @js($items), scheduleSubmit() { if (this.disabled) return; this.filtering = true; clearTimeout(this.submitTimer); this.submitTimer = setTimeout(() => this.$el.closest('form')?.requestSubmit(), 400); } }"
...
>
    ...
    <div class="relative">
        <input
            type="text"
            x-model="search"
            @input="if (!disabled) selected = items.find(item => item.label === search)?.id ?? ''"
            ...
            :disabled="disabled"
        >
        <button type="button" @click="if (!disabled) open = !open" :disabled="disabled" class="absolute inset-y-0 end-0 flex w-10 items-center justify-center text-zinc-500 disabled:cursor-not-allowed disabled:opacity-50" aria-label="Toggle {{ $label }} options">
        ...
        <div x-show="open && !disabled" x-cloak class="absolute z-50 mt-1 max-h-64 w-full overflow-y-auto rounded-lg border border-slate-300 bg-white p-1 shadow-xl dark:border-zinc-600 dark:bg-zinc-900">
