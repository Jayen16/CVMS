...
<div class="mt-2 border-t border-slate-200 pt-3 dark:border-zinc-700">
    ...
    <div class="mt-3 overflow-x-auto"><table class="app-table"><thead><tr><th class="px-3 py-2">Name</th><th class="px-3 py-2">Email</th><th class="px-3 py-2">Status</th><th class="px-3 py-2">Actions</th></tr></thead><tbody>
        @forelse ($users as $user)
            ...
            <tr class="app-table-row"><td class="px-3 py-2 font-medium">{{ $user->name }}</td><td class="px-3 py-2">{{ $user->email }}</td><td class="px-3 py-2">{{ $user->is_active && $user->invitation_accepted_at ? 'Active' : 'Pending' }}</td><td class="whitespace-nowrap px-3 py-2"><div class="flex items-center gap-2"><form method="POST" action="{{ route('locations.users.remove', $user) }}">@csrf<button class="app-button-danger !px-2 !py-1 !text-xs">Remove</button></form><flux:modal.trigger name="{{ $modalName }}"><flux:button type="button" size="base" class="min-w-28">Reassign</flux:button></flux:modal.trigger></div></td></tr>
