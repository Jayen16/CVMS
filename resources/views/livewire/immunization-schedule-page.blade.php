<div class="app-page">
    ...
    <section class="app-card overflow-hidden"><div class="app-card-header flex flex-wrap items-center justify-between gap-3"><div><h2 class="app-card-title">Child schedule checklist</h2><p class="mt-1 text-sm text-zinc-500">{{ $rows->count() }} of {{ $totalMatching }} matching children shown</p></div><div class="flex flex-wrap items-center gap-2"><label class="flex items-center gap-2 text-sm font-medium">Rows per page <select wire:model.live="perPage" class="app-input !w-auto"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select></label><a href="{{ route('reminder-history.index') }}" class="app-button-secondary">Reminder history</a><a href="{{ route('predictive-analytics.index') }}" class="app-button-secondary">View analytics</a></div></div>
